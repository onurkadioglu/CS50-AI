...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        neighbors = self.neighbors(cell)[0]
        mines_founded = self.neighbors(cell)[1]
        count -= mines_founded
        ...
        for sentence in self.knowledge:
            for mine in sentence.known_mines():
                new_mine_cells.add(mine)
            for safe in sentence.known_safes():
                new_safe_cells.add(safe)

        for mine in new_mine_cells:
            self.mark_mine(mine)
        for safe in new_safe_cells:
            self.mark_safe(safe)
    ...
    def make_random_move(self):
        ...
        random_move = possible_moves - self.moves_made - self.mines
